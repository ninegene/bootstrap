...
sudo add-apt-repository "deb http://$mirrorhost/mariadb/repo/10.0/ubuntu $version main"
...
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y mariadb-server-10.0
...
Hold/unhold a package for future upgrade using apt-mark if necessary
  $ sudo apt-mark hold mariadb-server-10.0
  $ sudo apt-mark hold mariadb-client-10.0
  $ sudo apt-mark unhold mariadb-server-10.0
  $ sudo apt-mark unhold mariadb-client-10.0
"
